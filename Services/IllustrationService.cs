...
namespace Frogmarks.Services
{
    public class IllustrationService : IIllustrationService
    {
        ...
        private readonly string _containerName = "illustration-thumbnails-dev"; // Blob Storage container, TODO: Use environment variable to determine.
        private readonly string _celContainerName = "illustration-cels-dev"; // Blob Storage container for cel/layer pixel data

        public IllustrationService(IApplicationDbContext context, IMapper mapper, BatchService batchService, IHttpContextAccessor httpContextAccessor, IBlobStorageProvider blobStorage)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<ResultModel<IllustrationDto>> GetIllustrationByUid(Guid uid)
        {
            try
            {
                var illustration = await _context.Illustrations.AsNoTracking()
                    .Include(i => i.Collaborators)
                        .ThenInclude(c => c.TeamUser)
                    .Include(i => i.Collaborators)
                        .ThenInclude(c => c.IllustrationRoles)
        ...
        public async Task<ResultModel<IllustrationStateDto>> SaveIllustrationState(long illustrationId, IllustrationStateDto stateDto)
        {
            try
            {
                ...
                illustration.CanvasData = stateDto.SceneGraph;
        ...
        public async Task<ResultModel<IllustrationStateDto>> LoadIllustrationState(long illustrationId)
        {
            try
            {
                var illustration = await _context.Illustrations
                    .AsNoTracking()
                    .Include(i => i.Layers.OrderBy(l => l.SortOrder))
                        .ThenInclude(l => l.Cels.OrderBy(c => c.Frame))
                    .FirstOrDefaultAsync(i => i.Id == illustrationId);

                if (illustration == null)
                    return new ResultModel<IllustrationStateDto>(ResultType.NotFound, "Illustration not found.");

                // If this is a v1 illustration, return the legacy CanvasData wrapped in v1 format
                if (illustration.SceneVersion <= 1)
                {
                    var legacyState = new IllustrationStateDto
                    {
                        Version = 1,
                        SceneGraph = illustration.CanvasData,
                        Animation = null,
                        Layers = new()
                    };
                    return new ResultModel<IllustrationStateDto>(ResultType.Success, resultObject: legacyState);
                }

                // V2 path: build full state DTO
                OnionSkinDto? onionSkin = null;
                if (!string.IsNullOrEmpty(illustration.OnionSkinConfig))
                {
                    try
                    {
                        onionSkin = JsonSerializer.Deserialize<OnionSkinDto>(illustration.OnionSkinConfig);
                    }
                    catch
                    {
                        // Malformed JSON — skip
                ...
                var layers = new List<LayerStateDto>();
                foreach (var layer in illustration.Layers)
                {
                    ...
                    // Static layer pixel data SAS URL
                    if (!layer.Animated && !string.IsNullOrEmpty(layer.PixelDataUrl))
                    {
                        var layerExt = layer.PixelFormat ?? "webp";
                        layerDto.PixelDataUrl = await _blobStorage.GetReadUrlAsync(_celContainerName, $"{illustrationId}/{layer.LayerId}.{layerExt}");
                    }
                    ...
                    foreach (var cel in layer.Cels)
                    {
                        ...
                        if (!string.IsNullOrEmpty(cel.PixelDataUrl))
                        {
                            var celExt = cel.PixelFormat ?? "webp";
                            celDto.PixelDataUrl = await _blobStorage.GetReadUrlAsync(_celContainerName, $"{illustrationId}/{cel.CelId}.{celExt}");
                        }
                ...
                {
                    Version = illustration.SceneVersion,
                    SceneGraph = illustration.CanvasData,
                    Animation = new AnimationStateDto
                    {
                        Enabled = illustration.AnimationEnabled,
                        FrameCount = illustration.FrameCount,
                        Fps = illustration.Fps,
                        LoopMode = illustration.LoopMode,
                        PlayRangeStart = illustration.PlayRangeStart,
                        PlayRangeEnd = illustration.PlayRangeEnd,
                        OnionSkin = onionSkin
                    },
                    Layers = layers
                };
        ...
        public async Task<ResultModel<string>> UploadCelPixelData(long illustrationId, string celId, IFormFile pixelData, int? width, int? height, string? format)
        {
            try
            {
                ...
                // Update the cel DB row with the URL and dimensions
        ...
        public async Task<ResultModel<string>> DeleteCel(long illustrationId, string celId)
        {
            try
            {
                ...
                {
                    // Swallow blob errors; the DB row is the source of truth
                }
